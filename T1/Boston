...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--input", type=Path, default=Path("data/Boston.tif"))
...
def main() -> None:
    ...
    for item in results:
        ...
        block = (
            f"\n===== Band {band} =====\n"
            f"Mean:   {item['mean']:.{args.precision}f}\n"
            f"Std:    {item['std']:.{args.precision}f}\n"
            f"Max:    {item['max']:.{args.precision}f}\n"
            f"Min:    {item['min']:.{args.precision}f}\n"
        )
        print(block, end="")
        lines.append(block)

    output_path = Path("T1/output.txt")
